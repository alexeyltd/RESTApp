package restful.security.handler;
...
public class CustomAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
    ...
    protected String determineTargetUrl(Authentication authentication) {
         ...
         */
        if (authorities.contains(new UserRole("ADMIN"))){
            return "/admin";
        }
        else if (authorities.contains(new UserRole("USER"))) {/**Если коллекция ролей у юзера не содержит роль ADMIN , но содержит роль USER - отправляет на url /user
         */
            return "/user";
        } else {
